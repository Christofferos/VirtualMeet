...
export const Game = ({
  ...
  const initializePlayingField = (state: any, wallDim: number) => {
    ...
    setGameState((prevState: any) => ({
      ...prevState,
      walls: { solid: state.walls.solid, movable: state.walls.movable },
    }));
  };
  ...
  const createGameState = useCallback((scoreInput: [number, number]) => {
    const wallDim = 4;
    const gameState = {
      players: [
        {
    ...
    initializePlayingField(gameState, wallDim);
    return gameState;
  }, []);

  const randomFood = (state: any) => {
    const food = {
  ...
  const initializeGameWindow = useCallback(() => {
    initGame([0, 0]);
    // if (!canvas.current || !context) return;
    // const contextTemp = context;
    // contextTemp.fillStyle = BG_COLOR;
    // contextTemp.fillRect(0, 0, CANVAS_WIDTH, CANVAS_WIDTH);
  ...
  }, [initGame, keypressed, keyreleased]);

  const collision = useCallback(
    (player: any, checkForMovableWall: boolean) => {
      let x1 = 0;
      let x2 = 0;
      let y1 = 0;
      let y2 = 0;
  ...
  const gameLoop = useCallback(
    (state: any) => {
      if (!state) return;
      if (!state.players) return alert(state);
  ...
  const handleGameState = useCallback(() => {
    // console.log('1');
    // console.log(gameStateTop);
    if (!gameStateTop) return;
    // console.log('HITHIT gameStateTop : ', gameStateTop);
    // alert('PAINT');
    setGameState(gameStateTop);
  }, [gameStateTop]);

  const startGameInterval = useCallback(
    (roomName: string) => {
      // console.log('GAMESTATE SHOULD BE UPDATED', gameState);
      let intervalId = setInterval(() => intervalIdProcedure(gameState), 1000 / FRAME_RATE);

      function intervalIdProcedure(gameState: any) {
        let winner = gameLoop(gameState);
        if (!winner) {
          // console.log('HITHIT: ', gameState);
          ...
          if (Math.max(gameState.scores.P1, gameState.scores.P2) < WINNING_SCORE) {
            setGameState(initGame([gameState.scores.P1, gameState.scores.P2]));
            setTimeout(
              () =>
    ...
    [gameState, gameLoop, emitGameState, handleGameState, emitGameOver, initGame],
  );

  useEffect(() => {
    // if (!context) return setContext(canvas.current.getContext('2d'));
    if (gameState) return startGameInterval(gameCode);
    initializeGameWindow();
  }, [gameCode, gameState, initializeGameWindow, startGameInterval]);
  ...
  const keyPressedUpdate = useCallback(
    ...
    [collision, wallDropAllowed],
  );

  const keyReleasedUpdate = useCallback((keyCode: any, player: any) => {
    if (player.directionPreference.includes(keyCode)) {
      ...
      >
        <div id="gameScreen" style={{ height: '90vh' }}>
          ...
          >
            ...
            <span className="scores text-white" style={{ display: 'flex', flexDirection: 'row' }}>
